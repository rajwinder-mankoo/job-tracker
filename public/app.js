...
function render() {
 ...
 if(view==='list') $('results').innerHTML=`<div class="table-wrap"><table><thead><tr><th>Company / Role</th><th>Priority</th><th>Location</th><th>Status</th><th>Visa fit</th><th>Date found</th><th></th></tr></thead><tbody>${filtered.map(j=>`<tr><td><div class="job-cell"><span class="avatar">${escape(j.company.slice(0,2).toUpperCase())}</span><div><button class="job-title" data-job="${escape(j.id)}">${escape(j.company||'Unnamed company')}</button><p>${escape(j.title)}</p></div></div></td><td>${priorityBadge(j.priority)}</td><td class="location">${escape(j.location||'Not recorded')}</td><td>${badge(j.status)}</td><td><span class="visa ${j.visa==='Good Fit'?'good':''}">${escape(j.visa||'Not reviewed')}</span></td><td class="date">${escape(dateText(j.found))}</td><td><button class="text-button" data-job="${escape(j.id)}" aria-label="View ${escape(j.company)} details">View ↗</button></td></tr>`).join('')}</tbody></table></div>`;
...
function showDetail(id) {
 ...
 $('detail-content').innerHTML=`${badge(j.status)} ${priorityBadge(j.priority)}<h2>${escape(j.company)}</h2><p class="detail-role">${escape(j.title)}</p><p class="detail-location">${escape(j.location)}</p>${j.url?`<a class="button primary" href="${escape(j.url)}" target="_blank" rel="noopener noreferrer">Open job posting ↗</a>`:''}<dl>${[['Priority',j.priority],['Visa compatibility',j.visa],['Date found',dateText(j.found)],['Date applied',dateText(j.applied)],['Follow-up date',dateText(j.followup)],['Resume version',j.resume],['Cover letter',j.cover]].map(([key,value])=>`<div><dt>${key}</dt><dd>${escape(value||'Not recorded')}</dd></div>`).join('')}</dl><h3>CPT / OPT / sponsorship</h3><p class="notes">${escape(j.sponsorship||'No notes yet.')}</p><h3>Research & notes</h3><p class="notes">${escape(j.notes||'No notes yet.')}</p><p class="detail-foot">To update this application, edit your Google Sheet or ask Muse. Changes appear here automatically.</p>`;
